<%-- 
    Document   : booklist
    Created on : 15.05.2019, 19:29:17
    Author     : Данил
--%>
...
<jsp:useBean id="readerController" class="com.library.controller.ReaderController" scope="session"></jsp:useBean>
    ...
    <html>
        ...
        <body>
            ...
            <ul id="navbar">
                <li><button name="reader" type="submit" value="1">На головну</button></li>
                <li><button name="exit" type="submit" value="1">Вихід</button></li>
            </ul>
        </form>
        <h2>Перелік книг, які можна замовити</h2>
        <table width ="100%">
            <tr>
                <td>
                        ...
                        <table  border ="1" class="simple-little-table">
                            ...
                            <c:forEach items="<%=request.getAttribute("allBooks")%>" var = "books">
                                <tr>
                                    ...
                                    <td><button name="choise" type="submit" value="${books.getId()}">Обрати</button></td>
                                </tr>
                            </c:forEach>
                        </table>        
                </td>
                    ...
                    <form action="${pageContext.request.contextPath}/Reader" method="Post">     
                        <ul>
                            <li><button name = "booklist" value="${genre.getId()}">Всі жанри</button></li>
                                <c:forEach items="<%=readerController.getAllGenres()%>" var = "genre">
                                <li><button name = "genre" value="${genre.getId()}">${genre.getName()}</button></li>
                                </c:forEach>
                        </ul>
                    </form>
                </td>
